...
import EditFaultScreen from '../Supervisor/EditFaultScreen';
...
describe('EditFaultScreen', () => {
  ...
  it('renders without crashing while loading', () => {
    const { queryClient, toJSON } = renderWithProviders(<EditFaultScreen />);
    expect(toJSON()).toBeTruthy();
    queryClient.clear();
  });

  it('loads the fault and prefills the form from getFaultById/getFaultCreationData', async () => {
    const { queryClient, findByText, findByDisplayValue } = renderWithProviders(<EditFaultScreen />);

    expect(await findByText('Guardar')).toBeTruthy();
    expect(await findByText('Grúa 1')).toBeTruthy();
    expect(await findByDisplayValue('Falla original')).toBeTruthy();
    queryClient.clear();
  });
  ...
  it('shows the loading error screen when the fault fails to load', async () => {
    ...
    const { queryClient, findByText } = renderWithProviders(<EditFaultScreen />);
