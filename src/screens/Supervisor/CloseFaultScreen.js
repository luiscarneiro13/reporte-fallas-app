...
export default function CloseFaultScreen() {
  ...
  const modals = {
    ...
    activityBy:  { setter: setActivityBy,  options: MOCK_EMPLOYEES  },
  };

  return (
    <ScreenContainer style={{ paddingTop: insets.top }}>
      <LinearGradient colors={['#1A3A6B', '#276749']} style={styles.header}>
      ...
      <ScrollContent paddingBottom={120}>
        ...
        <SelectField label={t('faults.reported_by')}    value={reportedBy?.label}  onPress={() => setActiveModal('reportedBy')}  required />
        <SelectField label={t('faults.equipment')}      value={equipment?.label}   onPress={() => setActiveModal('equipment')}   required />
        <SelectField label={t('faults.service_area')}   value={serviceArea?.label} onPress={() => setActiveModal('serviceArea')} required />
        <SelectField label={t('faults.fault_status')}   value={faultStatus?.label} onPress={() => setActiveModal('faultStatus')} required />
        <SelectField label={t('faults.spare_part_status')} value={sparePart?.label} onPress={() => setActiveModal('sparePart')} required />
        ...
        <View style={styles.fieldGroup}>
          ...
          <TextInput style={styles.textInput} value={completedExec} onChangeText={setCompletedExec} placeholder="dd-mm-yyyy" placeholderTextColor="#a0aec0" />
        </View>

        <SelectField label={t('faults.activity_performed_by')} value={activityBy?.label} onPress={() => setActiveModal('activityBy')} required />

        <View style={styles.fieldGroup}>
          <Text style={styles.label}>{t('faults.activities_performed')}<Text style={styles.req}> *</Text></Text>
          <TextInput style={[styles.textInput, styles.textArea]} value={activities} onChangeText={setActivities} multiline textAlignVertical="top" />
        </View>
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={styles.btnClose} onPress={() => navigation.goBack()} activeOpacity={0.85}>
